...
#define DEFAULT_ALBEDO "default_albedo"
#define DEFAULT_NORMAL "default_normal"
...
private:
	...
	D3D12_CPU_DESCRIPTOR_HANDLE m_uiTextureCurrentDecriptorOffset = {}; // Current offet in heap memory for mesh textures
	D3D12_GPU_DESCRIPTOR_HANDLE m_uiTextureCurrentGPUDecriptorOffset = {}; // Current GPU offet in heap memory for mesh textures(with the heap in Shader Visible)
	UINT m_uiSRVSizeInHeap = 0; // Size of one RTV in heap memory, used for updating the offset

	UINT m_uiSRVTextureInFlightOffset = 0; // Index of where textures in flight (for mesh materials) start

	ID3D12Device* m_pDevice = nullptr; // A pointer to the device once initialized;

	void DecreaseDescriptorOffset();
	...
	bool m_bTextureForRenderingDirty = false;

public:

	void Initialize(ID3D12Device* a_pDevice, UINT a_uiNbDecriptors);
	...
	void SetTextureForRenderingDirty(bool a_bDirty);
	bool IsTextureForRenderingDirty() const;
	void RegisterTexturesForRendering(std::map<UINT, D3DTexture*> a_oTextures, D3DTextureAddress*& a_rsTexturesAddress); // This function copies descriptors of textures used by meshes in a contiguous way to allow using a single descripor table. Only done once when every mesh in the scene is loaded.
